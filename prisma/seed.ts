...
async function main() {
  ...
  const project = await prisma.project.upsert({
    where: {
      name_organizationId: { name: 'Demo Project', organizationId: org.id },
    },
    update: {},
    create: {
      name: 'Demo Project',
  ...
  await prisma.projectMember.upsert({
    where: {
      projectId_memberId: { projectId: project.id, memberId: ownerMember.id },
    },
    update: {},
    create: {
  ...
  for (const s of statuses) {
    const status = await prisma.taskStatus.upsert({
      where: {
        projectId_position: { projectId: project.id, position: s.position },
      },
      update: {},
